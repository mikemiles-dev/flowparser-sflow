...
pub fn parse_extended_gateway(input: &[u8]) -> IResult<&[u8], ExtendedGateway> {
    ...
    let mut as_path_segments = Vec::with_capacity(as_path_count as usize);
    let mut input = input;
    ...
    let mut communities = Vec::with_capacity(communities_count as usize);
    let mut input = input;
